#!/bin/bash
# @Author: (rockops)[guofzhao@163.com]
# @Date: 2015/06/11
...
function check_start_log() {
    ...
    do
        ...
        if [ "${line_num}"x == x ]; then
            ...
            head -n $line_num $STDOUT_FILE | grep -v DEBUG | grep -v  "ERROR StatusLogger No log4j2 configuration file found" |grep -w "ERROR\|FATAL" >>/dev/null 2>&1
            return $?
        fi
    done
}
